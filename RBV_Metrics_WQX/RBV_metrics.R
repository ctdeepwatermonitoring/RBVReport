...
write.csv(new_taxa_confirm, "rbv_newtaxa_2023.csv", row.names = FALSE)
...
upload_test <- upload_test[!duplicated(upload_test[c("Name")]), ]#stuff that i need to update first to have 2023 data in wqx
...
write.csv(upload_test, "2023_outdated_names.csv")
...
colnames(merged_list)[colnames(merged_list) == "SID..Station"] <- "AWQ..Station"
merged_list <- merge(merged_list, awx_stations, by = "AWQ..Station")

#####making rbv final id category######
rbv_cat_table <- merged_list
rbv_cat_table <- rbv_cat_table[c("CTDEEPLablD", "AWQ..Station", "Stream", "munName",
...
rbv_cat_table <- rbv_cat_table[!duplicated(rbv_cat_table[c("CTDEEPLablD", "RBVFinalIDCat")]), ] #counting one finalid cat per sample

######presence/absence table for report######
rbv_cat_pivot <- rbv_cat_table
rbv_cat_pivot <- rbv_cat_pivot[c("CTDEEPLablD","AWQ..Station","Stream", "munName", "DATE_COL", "RBVFinalIDCat")] 
...
rbv_cat_pivot <- rbv_cat_pivot[c("CTDEEPLablD","AWQ..Station","Stream_Mun","DATE_COL", #just to look at, not write
...
#fixing names
#colnames(volmon_grp)[colnames(volmon_grp) == "Field.ID"] <- "CTDEEPLablD" 
#rbv_cat_pivot <- merge(rbv_cat_pivot, volmon_grp, by = "CTDEEPLablD") #adding org name to sample row
...
all_sites <- merged_list %>% #reference list of all unique site + date combos
  group_by(CTDEEPLablD, AWQ..Station, DATE_COL) %>%
  summarize() 
...
sites_with_no_most_wanted <- all_sites %>% 
  anti_join(sites_with_most_wanted, by = c("CTDEEPLablD", "AWQ..Station", "DATE_COL")) %>% #looking for what isnt in most wanted but is in all sites
...
final_site_metrics <- final_site_metrics[c("CTDEEPLablD", "AWQ..Station", "WaterbodyName", "munName", "DATE_COL", "RBV_most_wanted_count")]
colnames(final_site_metrics)[colnames(final_site_metrics) == "munName"] <- "Municipality"
write.csv(final_site_metrics, "RBV_Summary_2023.csv", row.names = FALSE)
...
most_wanted_by_site <- most_wanted_by_site[c("CTDEEPLablD", "AWQ..Station", "Stream", "DATE_COL", "FAMILY.x", "finalID.x")]
...
write.csv(most_wanted_by_site, "RBV_MostWantedBySite_2023.csv", row.names = FALSE)
...
colnames(labid)[colnames(labid) == "CTDEEPLablD"] <- "sampleID"
...
awx_stations <- read.csv("awx_stations_webservice.csv")
new_sites <- read.csv("RBV_Summary_2023.csv") #stationid and metrics results
sampleinfo <- read.csv("RBV_2023_Samples.csv") 
...
#####metrics upload file#####
upload_metrics <- merge(sampleinfo, new_sites, by = "CTDEEPLablD") #to preserve unique samples
#need to make: activity ID (accession# + _TaxaMostW)
#activity metric comment (= deep field id: / lab id:)
upload_metrics <- upload_metrics[c("SID..Station", "CTDEEPLablD", "DATE_COL.x", "RBV_most_wanted_count", "LAB_SAMPLE_ID" )]
upload_metrics$'Activity ID' <- paste((gsub("[^0-9]+", "", upload_metrics$CTDEEPLablD)), '_TaxaMostW') #pulling out numeric values and adding text
upload_metrics$'Activity Metric Comment' <- paste('DEEP Field ID: ', upload_metrics$CTDEEPLablD, ' / Taxonomy Lab ID: ', upload_metrics$LAB_SAMPLE_ID)
upload_metrics$'Activity Metric Value' <- upload_metrics$RBV_most_wanted_count #there are duplicated value and score columns
colnames(upload_metrics)[colnames(upload_metrics) == "SID..Station"] <- 'Monitoring Location ID'
...
upload_bio <- upload_bio[c("SID..Station", "CTDEEPLablD", "DATE_COL", "finalID.y", "DEEPTaxaID", "LAB_NAME", "RBVCategory" )] #need to add: activity ID, analysis start date (= DATE_COL)
...
colnames(upload_bio)[colnames(upload_bio) == "SID..Station"] <- 'Monitoring Location ID'
upload_bio$'Activity ID' <- paste((gsub("[^0-9]+", "", upload_bio$CTDEEPLablD)), '_BioResults', sep = "") #this used to be an auto number but i am changing it :)
...
new_taxa <- new_taxa[c("SID..Station", "CTDEEPLablD", "DATE_COL", "finalID", "DEEPTaxaID.x", "LAB_NAME", "RBVCategory" )] #need to add: activity ID, analysis start date (= DATE_COL)
colnames(new_taxa)[colnames(new_taxa) == "SID..Station"] <- 'Monitoring Location ID'
new_taxa$'Activity ID' <- paste((gsub("[^0-9]+", "", new_taxa$CTDEEPLablD)), '_BioResults', sep = "") #this used to be an auto number but i am changing it :)
